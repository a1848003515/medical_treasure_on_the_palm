...
Component({
  ...
  data: {
    ...
    logged:false,
    value:'',
    recordSrc:"../../images/components/listen.png",
  },

  methods: {
    btnRecord(){
      var that=this
      if(that.data.isYuYin==false){
        this.setData({
          isYuYin:true
        })
      }
    ...
    async initRoom() {
      this.try(async () => {
        ...
        const { data: initList } = await db.collection(collection).where(this.mergeCommonCriteria()).orderBy('sendTimeTS', 'asc').get()
            ...
            wx.cloud.getTempFileURL({
              ...
              success: res => {
                ...
                innerAudioContext.onPlay(() => {
                  console.log('开始播放1')
                  that.setData({
                    recordSrc: "../../images/components/yuyin.gif"
                  })
                })
